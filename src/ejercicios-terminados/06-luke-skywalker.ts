...
import { switchMap, map, zipAll } from 'rxjs/operators';
...
(() =>{
    ...
    // No tocar ========================================================
    const SW_API = 'https://jsonplaceholder.typicode.com';
    const getRequest = ( url: string ) => ajax.getJSON<any>(url);
    // ==================================================================

    // Realizar el llamado al URL para obtener a Luke Skywalker
    getRequest(`${SW_API}/users/5`).pipe(
        // Realizar los operadores respectivos aquí
        switchMap( res => zip( of(res), getRequest(`${SW_API}/posts?userId=${res.id}`) )),
